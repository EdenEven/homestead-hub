...
import {
  MapPin, Tag, ArrowLeftRight, Plus, Search, X,
  Clock, ChevronDown, Loader2, AlertCircle, CheckCircle2,
  Sprout, Hammer, Beef, Leaf, Wrench, Package, Landmark, MoreHorizontal
} from "lucide-react";
...
// ── Contact modal ──────────────────────────────────────────────────────────
function ContactModal({ listing, onClose }: { listing: { title: string; posterName?: string | null; posterEmail?: string | null; contactMethod?: string | null }; onClose: () => void }) {
  return (
      ...
      <div className="w-full max-w-md p-6 rounded-sm" style={{ backgroundColor: "oklch(0.98 0.01 85)", border: "1px solid oklch(0.82 0.03 75)" }}>
        ...
        {listing.posterEmail ? (
          <div className="p-3 rounded-sm mb-3" style={{ backgroundColor: "oklch(0.93 0.025 80)", border: "1px solid oklch(0.82 0.03 75)" }}>
            ...
            <a href={`mailto:${listing.posterEmail}?subject=Trade Inquiry: ${encodeURIComponent(listing.title)}`}
              className="text-sm font-semibold underline" style={{ color: "oklch(0.38 0.12 220)", fontFamily: "'Source Serif 4', Georgia, serif" }}>
              {listing.posterEmail}
            </a>
          </div>
        ...
        <button onClick={onClose} className="w-full py-2 font-bold rounded-sm text-sm"
...
function ListingCard({ listing, onContact }: { listing: Listing; onContact: (l: Listing) => void }) {
  ...
  return (
    ...
    >
      {/* Category bar */}
      <div className="px-4 py-2 flex items-center gap-2" style={{ backgroundColor: cat.bg, borderBottom: `1px solid oklch(0.85 0.025 75)` }}>
      ...
      <section className="relative py-20 overflow-hidden">
        ...
        <div className="relative container">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <div className="inline-flex items-center gap-2 px-4 py-3 rounded-sm text-sm"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-3xl mx-auto">
            {[
              ...
              <div key={s.step} className="text-center">
                <div className="w-10 h-10 rounded-full flex items-center justify-center mx-auto mb-3 font-black text-lg"
                  style={{ backgroundColor: "oklch(0.22 0.06 145)", color: "oklch(0.96 0.025 85)", fontFamily: "'Playfair Display', Georgia, serif" }}>
                  {s.step}
                </div>
